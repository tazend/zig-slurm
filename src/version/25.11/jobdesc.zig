...
pub const JobSubmitDescription = extern struct {
    ...
    cpus_per_tres: ?CStr = @import("std").mem.zeroes([*c]u8),
    ...
    job_id_str: ?CStr = @import("std").mem.zeroes([*c]u8),
    job_size_str: ?CStr = @import("std").mem.zeroes([*c]u8),
    kill_on_node_fail: u16 = NoValue.u16,
    licenses: ?CStr = @import("std").mem.zeroes([*c]u8),
    licenses_tot: ?CStr = @import("std").mem.zeroes([*c]u8),
    mail_type: u16 = NoValue.u16,
    mail_user: ?CStr = @import("std").mem.zeroes([*c]u8),
    mcs_label: ?CStr = @import("std").mem.zeroes([*c]u8),
    mem_bind: ?CStr = @import("std").mem.zeroes([*c]u8),
    mem_bind_type: u16 = NoValue.u16,
    mem_per_tres: ?CStr = @import("std").mem.zeroes([*c]u8),
    name: ?CStr = null,
    network: ?CStr = @import("std").mem.zeroes([*c]u8),
    ...
    spank_job_env: ?[*]?CStr = @import("std").mem.zeroes([*c][*c]u8),
    spank_job_env_size: u32 = @import("std").mem.zeroes(u32),
    step_id: Step.ID = .{},
    submit_line: ?CStr = @import("std").mem.zeroes([*c]u8),
    task_dist: u32 = @import("std").mem.zeroes(u32),
    time_limit: u32 = @import("std").mem.zeroes(u32),
    time_min: u32 = @import("std").mem.zeroes(u32),
    tres_bind: ?CStr = @import("std").mem.zeroes([*c]u8),
    tres_freq: ?CStr = @import("std").mem.zeroes([*c]u8),
    tres_per_job: ?CStr = @import("std").mem.zeroes([*c]u8),
    tres_per_node: ?CStr = @import("std").mem.zeroes([*c]u8),
    tres_per_socket: ?CStr = @import("std").mem.zeroes([*c]u8),
    tres_per_task: ?CStr = @import("std").mem.zeroes([*c]u8),
    ...
    req_context: ?CStr = @import("std").mem.zeroes([*c]u8),
    req_switch: u32 = @import("std").mem.zeroes(u32),
    segment_size: u16 = @import("std").mem.zeroes(u16),
    selinux_context: ?CStr = @import("std").mem.zeroes([*c]u8),
    std_err: ?CStr = null,
    std_in: ?CStr = null,
    std_out: ?CStr = null,
    tres_req_cnt: ?[*]u64 = @import("std").mem.zeroes([*c]u64),
    wait4switch: u32 = @import("std").mem.zeroes(u32),
    wckey: ?CStr = @import("std").mem.zeroes([*c]u8),
    x11: u16 = @import("std").mem.zeroes(u16),
    x11_magic_cookie: ?CStr = @import("std").mem.zeroes([*c]u8),
    x11_target: ?CStr = @import("std").mem.zeroes([*c]u8),
    ...
    pub const AllocationResponse = extern struct {
        ...
        cpu_freq_max: u32 = 0,
        cpu_freq_gov: u32 = 0,
        cpus_per_node: ?[*]u16 = null,
        cpu_count_reps: ?[*]u32 = null,
        env_size: u32 = 0,
        environment: ?[*]?CStr = null,
        error_code: u32 = 0,
        gid: gid_t = 0,
        group_name: ?CStr = null,
        job_submit_user_msg: ?CStr = null,
        node_cnt: u32 = 0,
        node_list: ?CStr = null,
        ...
        uid: uid_t = 0,
        user_name: ?CStr = null,
        working_cluster_rec: ?*anyopaque = null,
    };
};
